...
while($line=<IN>){
	...
	if($type eq "homo"){
		if($ele2[0] eq "1/1"){
			$vcf_info{$inf[0]}.=$line;
			$snpc++ if($ele1[0] ne "INDEL");
			$indelc++ if($ele1[0] eq "INDEL");
		}
	}
	elsif($type eq "heter"){
		if($ele2[0] ne "1/1"){
...
close OA;
open OA, ">$output.log" or die "can not open file: $output.log\n";
print OA "$output\tSNP changed:\t$snpc\n";
print OA "$output\tINDEL changed:\t$indelc\n";
close OA;
